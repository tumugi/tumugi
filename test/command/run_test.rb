  ...
  sub_test_case '#execute' do
    ...
    sub_test_case 'logging' do
      test 'default log level is INFO' do
        @cmd.execute(@dag, worker_type: 'thread')
        assert_equal(Logger::INFO, @task.logger.level)
      end

      test 'log level is DEBUG when verbose flag is true' do
        @cmd.execute(@dag, verbose: true, worker_type: 'thread')
        assert_equal(Logger::DEBUG, @task.logger.level)
      end

      test 'quiet mode' do
        result = capture_stdout { @cmd.execute(@dag, quiet: true, worker_type: 'thread') }
        assert_equal('', result)
      end
    end
  end
